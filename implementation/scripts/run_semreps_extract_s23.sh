...
#SBATCH --output=/data/user/mzhang630/data/mllm/logs/sr_s23_%j.log

# Extract LLaVA features for sub-02/03 NEW train stimuli (not already extracted
# for sub-01). Two outputs, same format as the sub-01 npz (coco_ids + layer_{L}):
#   semreps_s23_image_features.npz   <- s23_new_image_ids.json + s23_new_image_imgmap.json
#   semreps_s23_caption_features.npz <- s23_new_caption_text.json
...
logger = logging.getLogger('sr_s23')
...
# ---- inputs ----
img_ids=[int(c) for c in json.load(open(f'{SEMREPS_DIR}/s23_new_image_ids.json'))]
imgmap=json.load(open(f'{SEMREPS_DIR}/s23_new_image_imgmap.json'))
img_ids=[c for c in img_ids if str(c) in imgmap and os.path.exists(f'{SEMREPS_DIR}/images/{imgmap[str(c)]}')]
cap_text=json.load(open(f'{SEMREPS_DIR}/s23_new_caption_text.json'))
...
np.savez(f'{OUT}/semreps_s23_image_features.npz', coco_ids=np.array(img_ids),
         **{f'layer_{L}':ifeat[L] for L in LAYERS})
logger.info(f'Saved semreps_s23_image_features.npz ({n})')
...
np.savez(f'{OUT}/semreps_s23_caption_features.npz', coco_ids=np.array(cap_ids),
         **{f'layer_{L}':cfeat[L] for L in LAYERS})
logger.info(f'Saved semreps_s23_caption_features.npz ({m})')
